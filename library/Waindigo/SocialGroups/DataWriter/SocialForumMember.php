...
class Waindigo_SocialGroups_DataWriter_SocialForumMember extends XenForo_DataWriter
{
    ...
    protected function _preSave()
    {
        ...
        if ($socialForumMember && $socialForumMember['social_forum_member_id'] != $this->get('social_forum_member_id')) {
            $this->error(new XenForo_Phrase('waindigo_already_member_of_social_group_socialgroups'));
        }
        ...
        if ($this->isChanged('is_approved') || $this->isChanged('is_invited')) {
            if (XenForo_Application::$versionId >= 1020000) {
                ...
                if (!Waindigo_SocialGroups_SocialForum::hasInstance()) {
                    Waindigo_SocialGroups_SocialForum::setup($this->get('social_forum_id'));
                }
            ...
            if ($this->get('is_approved') && !$this->get('is_invited')) {
                ...
                if (XenForo_Application::$versionId >= 1020000 &&
                     $options->waindigo_socialGroups_watchForumOnJoin != 'no' && !empty($socialForum['node_id'])) {
                    $sendEmail = ($options->waindigo_socialGroups_watchForumOnJoin == 'yes_with_email');
    ...
    protected function _preDelete()
    {
        if ($this->getOption(self::OPTION_CHECK_SOCIAL_FORUM_CREATOR)) {
            if ($this->get('is_social_forum_creator')) {
                $this->error(
                    new XenForo_Phrase('waindigo_social_forum_creator_is_not_able_to_leave_group_socialgroups'));
            }
        }
    }
