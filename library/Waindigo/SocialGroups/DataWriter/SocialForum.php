...
class Waindigo_SocialGroups_DataWriter_SocialForum extends XenForo_DataWriter
{
    ...
    protected function _getExistingData($data)
    {
        ...
        $socialForum = Waindigo_SocialGroups_SocialForum::getSocialForumModel()->getSocialForumById($socialForumId);
        if (!$socialForum) {
            return false;
        }
    ...
    protected function _preSave()
    {
        ...
        if (!$this->get('url_portion')) {
            if (!empty($xenOptions->waindigo_socialGroups_urlPortions['required'])) {
                $this->error(new XenForo_Phrase('waindigo_please_enter_a_url_portion_socialgroups'), 'new_url_portion');
            }
        }
        ...
        if ($this->isChanged('url_portion')) {
            if ($this->get('url_portion'))
            {
                ...
                {
                    $this->error(new XenForo_Phrase('waindigo_url_portions_must_be_unique_socialgroups'), 'new_url_portion');
                }
            }

            if ($this->getExisting('url_portion') && empty($xenOptions->waindigo_socialGroups_urlPortions['editable'])) {
                $this->error(new XenForo_Phrase('waindigo_url_portion_cannot_be_changed_once_set_socialgroups'), 'new_url_portion');
            }
        }
    }
    ...
    protected function _postSave()
    {
        ...
        if ($this->isInsert()) {
            ...
            $xenOptions = XenForo_Application::get('options');
            if ($xenOptions->waindigo_socialGroups_createToSecondary) {
                $createToSecondary = $xenOptions->waindigo_socialGroups_createToSecondary;
    ...
    protected function _postDelete()
    {
        ...
        foreach ($members as $member) {
            ...
            $dw->setOption(Waindigo_SocialGroups_DataWriter_SocialForumMember::OPTION_CHECK_SOCIAL_FORUM_CREATOR, false);
    ...
    {
        /* @var $writer Waindigo_SocialGroups_DataWriter_SocialForumMember */
        $writer = XenForo_DataWriter::create('Waindigo_SocialGroups_DataWriter_SocialForumMember');
        $writer->setExtraData(Waindigo_SocialGroups_DataWriter_SocialForumMember::DATA_SOCIAL_FORUM, $this->getMergedData());
        $writer->bulkSet(
            array(
    ...
    public function rebuildCounters()
    {
        ...
        $this->bulkSet(
            Waindigo_SocialGroups_SocialForum::getSocialForumModel()->getSocialForumCounters(
                $this->get('social_forum_id')));
    }
    ...
    public static function getSocialCategoryCacheItem($nodeId)
    {
        if (!self::isSocialCategoryCacheItem($nodeId)) {
            $socialCategory = XenForo_Model::create('Waindigo_SocialGroups_Model_SocialCategory')->getForumById($nodeId);
            if (!$socialCategory) {
    ...
    protected function _verifyUrlPortion(&$data)
    {
        ...
        if (!preg_match('/^[a-z0-9_\-]+$/i', $data)) {
            $this->error(new XenForo_Phrase('waindigo_please_enter_url_portion_using_alphanumeric_socialgroups'),
                'new_url_portion');
            return false;
        }

        if ($data === strval(intval($data)) || $data == '-') {
            $this->error(new XenForo_Phrase('waindigo_url_portions_contain_more_numbers_hyphen_socialgroups'),
                'new_url_portion');
            return false;
        }
