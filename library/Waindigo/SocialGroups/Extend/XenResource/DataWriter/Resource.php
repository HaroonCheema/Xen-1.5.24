 ...
 */
class Waindigo_SocialGroups_Extend_XenResource_DataWriter_Resource extends XFCP_Waindigo_SocialGroups_Extend_XenResource_DataWriter_Resource
{
    ...
    protected function _insertDiscussionThread($nodeId, $prefixId = 0)
    {
        ...
        if ($node['node_type_id'] == 'SocialCategory') {
            ...
            if (!empty($socialForum) && $socialForum['node_id'] == $node['node_id']) {
                Waindigo_SocialGroups_SocialForum::setup($socialForum);
                ...
                Waindigo_SocialGroups_SocialForum::setup($writer->getMergedData());
