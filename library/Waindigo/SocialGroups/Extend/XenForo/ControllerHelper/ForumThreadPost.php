 ...
 */
class Waindigo_SocialGroups_Extend_XenForo_ControllerHelper_ForumThreadPost extends XFCP_Waindigo_SocialGroups_Extend_XenForo_ControllerHelper_ForumThreadPost
{
    ...
    public function getNodeBreadCrumbs(array $forum, $includeSelf = true)
    {
        if (Waindigo_SocialGroups_SocialForum::hasInstance()) {
            $socialForum = Waindigo_SocialGroups_SocialForum::getInstance()->toArray();
            if (isset($socialForum['social_forum_id'])) {
                return $this->_controller->getModelFromCache('Waindigo_SocialGroups_Model_SocialForum')->getNodeBreadCrumbs(
