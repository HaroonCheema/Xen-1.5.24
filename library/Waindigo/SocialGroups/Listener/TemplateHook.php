...
class Waindigo_SocialGroups_Listener_TemplateHook extends Waindigo_Listener_TemplateHook
{
    ...
    protected function _getHooks()
    {
        return array(
            ...
            'resource_view_tabs',
            'waindigo_social_forum_description_above_socialgroups'
        );
    }
    ...
    {
        $this->_appendTemplate('waindigo_account_sidebar_your_account_socialgroups');
    }

    protected function _threadCreateFieldsExtra()
    {
        if (isset($this->_hookParams['forum']['social_forum_id'])) {
            $this->_appendTemplate('waindigo_create_fields_extra_socialgroups');
        }
    }
    ...
    protected function _accountPersonalDetailsBiometrics()
    {
        ...
        if ($xenOptions->waindigo_socialGroups_primaryPostBit || $xenOptions->waindigo_socialGroups_secondaryPostBit) {
            ...
            $this->_prependTemplate('waindigo_account_social_forums_socialgroups', $viewParams);
        }
    }
    ...
    protected function _messageUserInfoText()
    {
        ...
        if ($viewParams['user']['primary_social_forum_id']) {
            ...
            $viewParams['user']['primary_social_forum_urls'] = Waindigo_SocialGroups_Template_Helper_SocialForum::getAvatarUrls(
                $socialForum);
        ...
        if (isset($viewParams['user']['secondary_social_forums']) && $viewParams['user']['secondary_social_forums']) {
            ...
            if (XenForo_Application::get('options')->waindigo_socialGroups_secondaryPostBit) {
                foreach ($viewParams['user']['secondary_social_forums'] as &$socialForum) {
                    $socialForum['urls'] = Waindigo_SocialGroups_Template_Helper_SocialForum::getAvatarUrls(
                        $socialForum);
                }
            }
        }
        $this->_appendTemplateAtSlot('message_user_info_text', 'waindigo_message_user_info_socialgroups', $viewParams);
    }

    protected function _memberViewTabsHeading()
    {
        $this->_appendTemplate('waindigo_member_view_tabs_heading_socialgroups');
    }

    protected function _memberViewTabsContent()
    {
        $this->_appendTemplate('waindigo_member_view_tabs_content_socialgroups');
    }

    protected function _navigationVisitorTabLinks2()
    {
        $this->_appendTemplate('waindigo_navigation_visitor_tab_links_socialgroups');
    }
    ...
    protected function _resourceViewTabs()
    {
        ...
        if ($resource['social_forum_id'] && empty($viewParams['thread'])) {
            $this->_appendTemplate('waindigo_resource_view_tabs_socialgroups', $viewParams);
        }
    }

    protected function _waindigoSocialForumDescriptionAboveSocialgroups()
    {
        $viewParams = $this->_fetchViewParams();
        if ($viewParams['socialForum']['social_forum_type'] == 'resource') {
            ...
            if ($resource && $resourceModel->canViewResourceAndContainer($resource, $resource)) {
                ...
                Waindigo_SocialGroups_SocialForum::getInstance()->setResource($resource);
            }
        }
    }
