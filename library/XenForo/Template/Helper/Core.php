...
class XenForo_Template_Helper_Core
{
	...
	*/
	...
	public static function helperUserBlurb(array $user, $includeUserTitle = true)
	{
		...
		{
			$user['locationCensored'] = XenForo_Helper_String::censorString($user['location'], null, 'x');

			if (XenForo_Application::getOptions()->geoLocationUrl)
			{
				$location = '<a href="'
					. XenForo_Link::buildPublicLink('misc/location-info', '', array('location' => $user['locationCensored']))
					. '" class="concealed" target="_blank" rel="nofollow noreferrer">'
					. htmlspecialchars($user['locationCensored'])
					. '</a>';
			}
			else
			{
				$location = $user['locationCensored'];
			}
