...
class XenGallery_Importer_PhotopostVb extends XenForo_Importer_Abstract
{
	...
	public function stepAlbums($start, array $options)
	{
		...
		$albums = $sDb->fetchAll(
			$sDb->limit('
				...
				LEFT JOIN ' . $prefix . 'users AS user ON
					(category.parent = user.userid)
				WHERE category.id > ?
					AND cattype = \'a\'
	...
	protected function _importAlbum(array $album, array $options)
	{
		...
		$user = false;
		$userId = isset($this->_userIdMap[$album['parent']]) ? $this->_userIdMap[$album['parent']] : 0;
		if ($userId)
		{
			$user = $xfDb->fetchRow('
	...
	public function stepMemberCategories($start, array $options)
	{
		...
		{
			$options['max'] = $sDb->fetchOne('
				SELECT MAX(photo.userid)
				FROM ' . $prefix . 'categories AS category
				LEFT JOIN ' . $prefix . 'photos AS photo ON
					(photo.cat = category.id)
				WHERE category.ismember = 1
			');
		}

		$categories = $sDb->fetchAll(
			$sDb->limit('
				SELECT photo.userid, photo.user, category.id
				FROM ' . $prefix . 'categories AS category
				LEFT JOIN ' . $prefix . 'photos AS photo ON
					(photo.cat = category.id)
				WHERE photo.userid > ?
					AND category.ismember = 1
				GROUP BY category.id, photo.userid
				ORDER BY category.id ASC
		', $options['limit']), $start);
		if (!$categories)
		{
		...
		foreach ($categories AS $category)
		{
			...
			$user = false;
			$userId = isset($this->_userIdMap[$category['userid']]) ? $this->_userIdMap[$category['userid']] : 0;
			if ($userId)
			{
				$user = $xfDb->fetchRow('
					SELECT user_id, username
			...
			if (!$user)
			{
				$user = array(
					'user_id' => 0,
					'username' => $this->_convertToUtf8($category['user'])
				);
			}
	...
	protected function _importMemberCatToAlbum(array $category, array $user, $albumTitle)
	{
		...
		$dates = $sDb->fetchCol('
			...
			WHERE cat = ?
				AND userid = ?
			ORDER BY date ASC
		', array($category['id'], $category['userid']));
		...
		$oldId = array(
			'c' => intval($category['id']),
			'u' => intval($category['userid'])
		);
	...
	public function stepMedia($start, array $options)
	{
		...
		$media = $sDb->fetchAll(
			$sDb->limit('
				SELECT photo.*, category.parent,
					category.cattype, category.private, category.ismember
		...
		foreach ($media AS $item)
		{
			...
			$exists = $model->mapMediaId($item['id']);
			if ($exists)
			{
				$total++;
				continue;
	...
	protected function _importMedia(array $item, array $options)
	{
		...
		$user = false;
		$userId = isset($this->_userIdMap[$item['userid']]) ? $this->_userIdMap[$item['userid']] : 0;
		if ($userId)
		{
			$user = $xfDb->fetchRow('
				SELECT user_id, username
		...
		if (!$user)
		{
			$user = array(
				'user_id' => 0,
				'username' => $this->_convertToUtf8($item['user'])
			);
		}
		...
		elseif ($item['cattype'] == 'c' && !$item['ismember'])
		{
		...
		elseif ($item['cattype'] == 'c' && $item['ismember'])
		{
			$oldId = serialize(array('c' => intval($item['cat']), 'u' => intval($item['userid'])));
	...
	protected function _importComment(array $comment, array $options)
	{
		...
		$user = false;
		$userId = isset($this->_userIdMap[$comment['userid']]) ? $this->_userIdMap[$comment['userid']] : 0;
		if ($userId)
		{
			$user = $xfDb->fetchRow('
				SELECT user_id, username
		...
		if (!$user)
		{
			$user = array(
				'user_id' => 0,
				'username' => $this->_convertToUtf8($comment['username'])
			);
		}

		$contentId = $model->mapMediaId($comment['photo']);
		$message = XenForo_Template_Helper_Core::helperSnippet(
			$this->_convertToUtf8($comment['comment'], true), 0, array('stripHtml' => true)
		);
	...
	protected function _importRating(array $rating, array $options)
	{
		...
		$userId = isset($this->_userIdMap[$rating['userid']]) ? $this->_userIdMap[$rating['userid']] : 0;
		$user = $xfDb->fetchRow('
			SELECT user_id, username
			FROM xf_user
			WHERE user_id = ?
