...
class XenGallery_Importer_PhotopostVbGallery extends XenForo_Importer_Abstract
{
	...
	protected function _importAlbum(array $album, array $options)
	{
		...
		$user = false;
		$userId = isset($this->_userIdMap[$album['catuserid']]) ? $this->_userIdMap[$album['catuserid']] : 0;
		if ($userId)
		{
			$user = $xfDb->fetchRow('
	...
	protected function _importMedia(array $item, array $options)
	{
		...
		{
			$this->_safeMode = $sDb->fetchOne('SELECT value FROM ' . $this->_prefix . 'ppgal_setting WHERE varname = \'gallery_insafemode\'');
		}

		$fileName = $item['originalname'] ? $item['originalname'] : $item['filename'];
		if ($this->_safeMode)
		{
		    $originalFilePath = sprintf('%s/%s', $this->_fileRoot, $fileName);
		}
		else
		{
			$splitId = implode('/', str_split($item['userid']));
		    $originalFilePath = sprintf('%s/%s/%s', $this->_fileRoot, $splitId, $fileName);
		}
		...
		elseif ($item['catuserid'] == 0 && $item['membercat'])
		{
			$oldId = serialize(array('c' => intval($item['catid']), 'u' => intval($item['userid'])));
	...
	public function stepCustomFields($start, array $options)
	{
		...
		foreach ($fields AS $field)
		{
			...
			$field['title'] = $this->_convertToUtf8($field['title']);
			$field['description'] = $this->_convertToUtf8($field['description']);
	...
	protected function _importField(array $field, array $options)
	{
		...
		$fieldChoices = array_map(array($this, '_convertToUtf8'), preg_split('/\r?\n/', trim($field['options']), -1, PREG_SPLIT_NO_EMPTY));
	...
	protected function _importFieldValues(array $item)
	{
		...
		foreach (array_keys($fieldIds) AS $fieldId)
		{
			...
			{
				$value = $this->_convertToUtf8($item['field' . $fieldId]);
