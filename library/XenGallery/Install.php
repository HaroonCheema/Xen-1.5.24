...
class XenGallery_Install
{
	...
	public static function stepVersions()
	{
		...
		if ($version < 901010570)
		{
			self::_executeQuery("
				INSERT IGNORE INTO xf_option
					(option_id, option_value, default_value, edit_format, edit_format_params, data_type, sub_options, can_backup, validation_class, validation_method, addon_id)
				VALUES
					('xengalleryAllowUploadFromUrl', '1', '', '', '', '', '', 1, '', '', 'XenGallery')
			");

			self::_executeQuery("
				ALTER TABLE xengallery_comment ADD INDEX user_id (user_id)
			");
		}

		if ($version < 901010770)
		{
			$sites = $db->fetchOne('
				SELECT option_value
				FROM xf_option
				WHERE option_id = ?
			', 'xengalleryMediaThumbs');
			...
			if ($sites)
			{
				...
				{
					// Weird situation whereby if a field was left blank, it would store the strlen of the ID.
					// Convert any values which are numeric (or blank) to be the default no thumb image.
					if (is_numeric($site) || !strlen($site))
					{
						$site = XenForo_Application::getOptions()->xengalleryDefaultNoThumb;
						$update = true;
					}
				}
				...
				if ($update)
				{
					self::_executeQuery("
						...
						SET option_value = ?
						WHERE option_id = ?
					", array(serialize($sites), 'xengalleryMediaThumbs'));
				}
			}
		}

		if ($version < 901011170)
		{
			self::_executeQuery("
		...
		if ($version < 901011270)
		{
			$exists = $db->fetchRow('SHOW INDEXES FROM xengallery_media WHERE Key_name = \'media_date\'');
			if (!$exists)
			{
				self::_executeQuery("
	...
	protected static function _getAlters($key = null)
	{
		...
		$alters[112]['xengallery_media_type_3'] = "
			UPDATE xengallery_media SET media_type = 'video_embed' WHERE media_type = 'video'
		";
