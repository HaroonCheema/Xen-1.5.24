...
class XenGallery_ControllerPublic_Album extends XenGallery_ControllerPublic_Abstract
{
	...
	public function actionView()
	{
		...
		$viewParams = array(
			...
			'canReport' => $userModel->canReportContent(),
	...
	public function actionMarkViewed()
	{
		...
		if ($this->isConfirmedPost())
		{
			...
			$mediaModel->markMediaIdsViewed($mediaIds);
	...
	public function actionCreate()
	{
		...
		if ($this->isConfirmedPost())
		{
			...
			if (!$canChangeViewPermission)
			{
				$viewMedia = $options->xengalleryAlbumViewPerm;
			}

			if (!$canChangeAddPermission)
			{
			...
			// Workaround to explicitly set this (view is set by default).
			$albumWriter->changeAlbumPrivacy('add');
