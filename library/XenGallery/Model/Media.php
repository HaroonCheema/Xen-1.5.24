...
class XenGallery_Model_Media extends XenForo_Model
{
	...
	public function markMediaViewed(array $media, array $viewingUser = null)
	{
		...
		$this->removeMediaIdFromUnviewedCache($media['media_id']);

		return true;
	}

	public function markMediaIdsViewed(array $mediaIds)
	{
		$this->standardizeViewingUserReference($viewingUser);
		...
		$timeQuoted = $db->quote(XenForo_Application::$time);

		$values = array();
		foreach ($mediaIds AS $mediaId)
		{
			$mediaIdQuoted = $db->quote($mediaId);
			$values[] = "($userIdQuoted, $mediaIdQuoted, $timeQuoted)";

			$this->removeMediaIdFromUnviewedCache($mediaId);
		}
	...
	public function removeMediaIdFromUnviewedCache($mediaId)
	{
		if (XenForo_Application::getOptions()->xengalleryUnviewedCounter['enabled'] && XenForo_Application::isRegistered('session'))
		{
			...
			{
				unset ($mediaUnviewed['unviewed'][$mediaId]);
			}
	...
	*/
	...
	public function sendAuthorAlert(array $content, $contentType, $action, array $options, array $extra = array(), $userIdKey = 'user_id')
	{
		...
		// Can contain cached phrases which we don't want to be serialized as extra_data so unset.
